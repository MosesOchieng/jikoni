...
const JWT_SECRET = process.env.JWT_SECRET || process.env.SESSION_SECRET || "dev-jikoni-secret-change-me";
...
function verifyToken(token) {
  ...
  if (!decoded.exp || decoded.exp < Date.now()) throw new Error("Expired token");
  return decoded;
}
...
// Email is intentionally a no-op unless an email provider is added later.
// The core app remains usable in Replit without SMTP credentials.
const mailer = {
  verify(callback) {
    callback(null, true);
  },
  sendMail(options, callback) {
    callback(null, { messageId: "development-mode" });
  },
};
...
mailer.verify((error, success) => {
  if (error) {
    ...
    console.log(process.env.SMTP_USER ? "Mailer verified and ready" : "Mailer running in development mode");
  }
});
